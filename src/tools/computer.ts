...
export function createComputerServer(
  ...
  server.tool(
    ...
    {
      cmd: z.string().describe(`\
The bash command to execute. Run blocking commands as daemons using \`&\` \
as there is no timeout mechanism.`),
    },
    async ({ cmd }) => {
      const c = await Computer.ensure(agent.toJSON().name);
      if(c.isErr()) {
        return errorToCallToolResult(
          new SrchdError("computer_run_error", "Publication not found")
        );
